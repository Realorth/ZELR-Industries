...
void Interaccion::colision(hombre& h, pared p)
{
	...
	float posy = h.GetPos().y;
	float alt = h.GetAltura() / 2;
	float ymin = posy +alt;
	float ymax = posy -alt;
	float y = p.GetLim1().y;
	float xmax = p.GetLim2().x;
	float xmin = p.GetLim1().x;*/
	//float xmax = p.limite2.x;
	//float dif = p.distancia(h.posicion, &dir) - h.altura;
	vector2D dir;
	float dif = p.distancia(h.posicion, &dir) - h.altura/15;
	if (dif <= 0.0f)
	{
		//vector2D v_inicial = h.velocidad;
		h.velocidad.y=0 ;
...
/*
	if ((h.GetPos().x > xmin && h.GetPos().x < xmax)&&(ymax>y && ymin<y)) {
		/*
		if (h.GetPos().x>xmin) {
			h.SetPos(xmin, h.GetPos().y);
		}
		else {
			h.SetPos(xmax, h.GetPos().y);
		}
		
		if (y<ymax) {
			h.SetPos(h.GetPos().x, ymax);
		}
		else {
			h.SetPos(h.GetPos().x, ymin);
		}
		//h.posicion.y = ymin+h.GetAltura()/2;
	...
	*/
	...
	//float xmax = p.limite2.x + h.altura / 2;
	//float xmin = p.limite1.x- h.altura / 2;
