...
bool ListaCorazones::agregar(BonusCorazon* b)
{
	...
	if ((numero < MAX_CORAZONES) && (comprueba == false)) {
...
void ListaCorazones::eliminar(BonusCorazon* b)
{
	for (int i = 0; i < numero; i++)
		if (lista[i] == b) {
			eliminar(i);
...
BonusCorazon* ListaCorazones::colision(hombre& h)
{
	for (int i = 0; i < numero; i++)
	{
...
BonusCorazon* ListaCorazones::operator[](int i)
{
